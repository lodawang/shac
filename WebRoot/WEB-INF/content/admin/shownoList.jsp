...
<div class="main">
  ...
  <div class="right">
    ...
    <div class="btn_frame">
      <div style="float:left"><h1>已领取编号记录      </h1></div>
      
    </div>
    
    <form name="searchForm" action="<s:url value="/admin/showNolist.do"/>" method="post" onsubmit="schFormOnSubmit();">
    <s:hidden name="page"></s:hidden>
      ...
      <table width="99%" border="0" cellpadding="0" cellspacing="0" class="table_list">
        ...
        <s:iterator value="mnoList" status="stat">
        <tr <s:if test="#stat.even==true">class="td_bg1"</s:if>>
          <td> <s:property value="#stat.count"/></td>
          <td> <s:property value="user.loginID"/></td>
          <td> <s:property value="jobno"/></td>
          <td> <s:if test='notype == "J"'>正式</s:if><s:if test='notype == "P"'>试制</s:if></td>
          <td style="width:600px;word-break:break-all"><s:property value="nos"/></td>
          <td>&nbsp; <s:property value="nocount"/></td>
          <td><s:date name="noTime" format="yyyy/MM/dd HH:mm"/>&nbsp;</td>
        </tr>
        </s:iterator>
      </table>
       ...
       <p class="page-inner" style="text-align:right;width:99%">
		...
		<pg:pager items="${totalCount}" url="${pageContext.request.contextPath}/admin/showNolist.do" index="center" maxPageItems="10" maxIndexPages="10" isOffset="<%=false%>" export="currentPageNumber=pageNumber" scope="request">
		<pg:index export="pageCount">
			...
			转到  <select onchange="selectedPage=this.value;jumpPage('<s:url value="/admin/showNolist.do"/>',selectedPage)">
