...
<div class="main">
  ...
  <div class="right">
    ...
    <div class="btn_frame">
      ...
      <font color="red">说明：本页只列出你有权修改的技术文档</font>
      </div>
      <div style="float:right">
      <input name="button" type="button" class="btn_ty" id="button" value="工艺管理"  onclick="window.location='factoryDocMngList.do?docType=2'"/>
      <input name="button" type="button" class="btn_ty" id="button" value="图纸管理"  onclick="window.location='factoryDocMngShow.do?docType=0'"/>      
      </div>
    </div>
    <form name="searchForm" action="factoryDocMngTechShow.do" method="post" onsubmit="schFormOnSubmit();">
       ...
       <th>序号</th>
        ...
        <tr <s:if test="#stat.even==true">class="td_bg1"</s:if>>
         ...
         技术文件
          ...
          <td align="center">
          	<a href="<s:url action="viewTaskTechFactory" namespace="/admin"><s:param name="id" value="task.id"/></s:url>" class="font_green"><strong><u>发放</u></strong></a>
          </td>
        </tr>
        </s:iterator>
      </table>
      <p class="page-inner" style="text-align:right;width:99%">
		<pg:pager items="${totalCount}" url="${pageContext.request.contextPath}/admin/factoryDocMngTechShow.do" index="center" maxPageItems="10" maxIndexPages="10" isOffset="<%=false%>" export="currentPageNumber=pageNumber" scope="request">
		<pg:index export="pageCount">
			...
			转到  <select onchange="selectedPage=this.value;jumpPage('<s:url value="/admin/factoryDocMngTechShow.do"/>',selectedPage)">
