  ...
  <div class="right">
    ...
    <div class="btn_frame">
      <div style="float:left">
      	<form name="searchForm" action="docLatest.do" method="post">
      	<s:hidden name="page"></s:hidden>
        <s:select theme="simple" onchange="this.form.submit();" cssClass="u38" cssStyle="width:152px" list="#{'正式生产':'正式生产','试生产':'试生产','停产':'停产','取消':'取消'}" name="deptItem.task.processIn"  listKey="key" listValue="value"></s:select>
        </form>
      </div>
...
<tr>
        ...
        工艺文档
          ...
          <td>
          	<a href="<s:url action="docPreview.do" namespace="/admin"><s:param name="id" value="id"/></s:url>" class="font_green"><strong><u>查看</u></strong></a>
          </td>
        </tr>
        </s:iterator>
      </table>
 	 <p class="page-inner" style="text-align:right;width:99%">
		<pg:pager items="${totalCount}" url="${pageContext.request.contextPath}/admin/docLatest.do" index="center" maxPageItems="10" maxIndexPages="10" isOffset="<%=false%>" export="currentPageNumber=pageNumber" scope="request">
		<pg:index export="pageCount">
			...
			转到  <select onchange="selectedPage=this.value;jumpPage('<s:url value="/admin/docLatest.do"/>',selectedPage)">
