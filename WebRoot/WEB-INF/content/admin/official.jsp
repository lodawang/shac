...
function doFormOnSubmit(){
		...
		if(confirm('是否确定把'+$("#hiddenpickdate").val()+'的数据转正式?')){
		   return true;
		}
          return false;
    }    
...
<div class="main">
  ...
  <div class="right">
    ...
    <div class="btn_frame">
      ...
      <form name="offForm" action="dooff.do" method="post" onsubmit="return doFormOnSubmit()">
         <input name="button" type="submit" class="btn_ty" id="button" value="转正式" />
          <s:hidden name="hiddenpickdate"></s:hidden>
      </form>      
      </span>
       <form name="offFormTwo" action="clearoff.do" method="post">
         <input name="buttontwo" type="submit" class="btn_ty" id="buttontwo" value="清理数据" />          
          ${flagmessage}
      </form>      
    </div>
    <form name="searchForm" action="tooff.do" method="post" onsubmit="return schFormOnSubmit()">
    <s:hidden name="page"></s:hidden>
    <table width="99%" border="0" cellpadding="0" cellspacing="0" class="table_form">
      <tr>
        <th>日期：</th>
        <td>
        <s:textfield name="pickdate" cssClass="u23" id="pickdate" onClick="WdatePicker()"/>
        </td>
