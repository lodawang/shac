...
<div class="main">
  ...
  <div class="right">
    <div class="text_c">
      当前位置：首页 &gt; 系统设置 &gt; 发放规则       | <a href="batchConfirmReceive.do">批量接收</a>
    </div>
    <div class="btn_frame">
      <div style="float:left">
      <form action="<s:url value="/admin/issueRuleList.do"/>" method="post">
     文档类型：<s:select theme="simple" cssClass="u38" cssStyle="width:152px" list="#{'0':'图纸','1':'技术文件','2':'工艺文档'}" name="issueRule.docType"  listKey="key" listValue="value" headerKey="" headerValue="请选择"></s:select>
     <input name="button" type="submit" class="btn_gray" id="button" value="查询" />
      </form>
      </div>
      <div style="float:right">
      <input name="button" type="button" class="btn_ty" id="button" value="新增"  onclick="window.location='issueRuleEdit.do'"/>
      </div>
    </div>
    <table width="99%" border="0" cellpadding="0" cellspacing="0" class="table_list">
        ...
        <s:iterator value="ruleList" status="stat">
        <tr <s:if test="#stat.even==true">class="td_bg1"</s:if> <s:if test="id==#parameters.id[0]">style="background:orange"</s:if>>
          <td> <s:property value="#stat.count"/> </td>
          <td>
          	<s:if test='docType=="0"'>图纸</s:if>
          	<s:if test='docType=="1"'>技术文件</s:if>
          	<s:if test='docType=="2"'>工艺文档</s:if>&nbsp;
          </td>
          <td> 
          <s:property value="processIn"/>&nbsp;
          </td>
          <td> 
          <s:property value="procMode"/>&nbsp;
          </td>
          <td> 
          <s:property value="procDocClass"/>&nbsp;
          </td>
          <td><a href="<s:url action="issueRuleEdit" namespace="/admin"><s:param name="id" value="id"/></s:url>" class="font_green"><strong><u>修改</u></strong></a>
          	  <a onclick="return confirm('确认删除该发放规则?');" href="<s:url action="delIssueRule" namespace="/admin"><s:param name="id" value="id"/></s:url>" class="font_green"><strong><u>删除</u></strong></a>
          </td>
        </tr>
        </s:iterator>
      </table>
