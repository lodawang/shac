...
<div class="main">
  ...
  <div class="right">
    ...
    <div class="btn_frame">
      ...
      <input name="button" type="button" class="btn_ty" id="button" value="重置附件"  onclick="window.location='reloadfile.do'"/>
      <input name="button" type="button" class="btn_ty" id="button" value="反馈意见"  onclick="window.location='adviceList.do'"/>
      <input name="button" type="button" class="btn_ty" id="button" value="老TC任务"  onclick="window.location='issueTaskListOld.do'"/>
    ...
    <form name="searchForm" action="listTCManaDocuNew.do" method="post" >    
    ...
    <table width="99%" border="0" cellpadding="0" cellspacing="0" class="table_list">
        ...
        <s:iterator value="tasktcList" status="stat">
        <!--  <tr <s:if test="#stat.even==true">class="td_bg1"</s:if> <s:if test="missup==1">bgcolor="yellow"</s:if>>-->
        <tr <s:if test="status==1">bgcolor="yellow"</s:if>>
          <td <s:if test="missup==1">bgcolor="red"</s:if> > <s:property value="#stat.count"/> </td>
          <td <s:if test="erroattach==1">bgcolor="gray"</s:if> > <s:property value="partid"/> </td>
          <td>
          ${erroattach}
          <s:if test="docType==0">
          产品图纸 
         ...
         技术文件
          ...
          <td>
          <a href="<s:url action="editDocuTC" namespace="/admin"><s:param name="id" value="id"/></s:url>" class="font_green"><strong><u>属性修改</u></strong>
          <a href="<s:url action="delTcTaskNew" namespace="/admin"><s:param name="id" value="id"/></s:url>" class="font_green" onclick="return confirm('确认删除？')"><strong><u>删除</u></strong>
          </a>
          </td>
        </tr>
        </s:iterator>
